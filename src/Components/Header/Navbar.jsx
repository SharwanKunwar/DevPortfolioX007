...
function Navbar() {
  ...
  return (
    <div className='md:sticky md:top-0 bg-gray-400/30 backdrop-blur-lg w-full md:h-[100px] flex justify-center md:items-center items-start shadow-sm z-100'>
      <nav className='bg-white/30 drop-shadow-2xl border-2 border-white/30 md:w-[90%] md:h-[70%] h-[70px] py-10 md:py-0 md:rounded-full flex justify-between items-center shadow-md'>
        ...
        <div className='md:w-[10%] w-[20%] h-full flex justify-center items-center'>
          <section className='w-full h-full flex justify-start pl-3 items-center '>
            ...
            className=' md:w-[55px] w-[80%] hover:rotate-360 transition-all duration-300' src="icons/logo01.png" alt="logo" /> 
            </a>
        ...
        <div className='w-full h-full md:flex justify-end items-center hidden'>
          <ul className='flex justify-evenly w-[50%] h-full items-center'>
            {MenuList.map((item, index) => {
              const path = '/' + item.toLowerCase(); // assume paths like /home, /about...
              ...
              return (
                <Link
                  ...
                  to={path}
                  className={`px-3 py-1 ${
                    isActive ? 'text-white text-[18px]' : 'text-black text-[18px] hover:border-b-1 border-black/30 '
                  }`}
                >
                  {item}
                </Link>
        ...
        <div className='md:w-[25%] w-[20%] h-full flex justify-center items-center'>
          <section className='hidden  w-full h-full md:flex justify-center items-center gap-5 text-white'>
             <a className='hover:border-b-1 py-2' href="#facebook" aria-label="Facebook"><FaFacebook size={23} color="#1877F2" /></a>
              <a className='hover:border-b-1 py-2' href="#instagram" aria-label="Instagram"><FaInstagram size={23} color="#E1306C" /></a>
              <a className='hover:border-b-1 py-2' href="#github" aria-label="GitHub"><FaGithub size={23} color="#333" /></a>
          </section>
      ...
      {/* Mobile Menu Popup */}
      {isOpen && (
        <div className='bg-black/30 backdrop-blur-sm absolute top-0 left-0 w-screen h-screen flex justify-center items-center'>
          <motion.div 
          initial={{
            // y:-200,
            scale:0,
                ...
                {MenuList.map((item, index) => {
                  ...
                  return (
                    <Link
                      key={index}
                      to={path}
                      className={`px-[30%] py-3 rounded-sm ${
